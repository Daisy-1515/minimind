...
#### 2.5 CausalLMOutputWithPast 类型兼容性修复
**位置**: `MokioMindForCausalLM.forward()` 第 827-841 行
**修复日期**: 2025-12-02
**问题**: `CausalLMOutputWithPast` 期望 `Cache | None` 类型，但传入了 `List[Optional[Tuple[torch.Tensor, torch.Tensor]]]`

**错误信息**:
```
无法将"List[Tuple[Tensor, Tensor] | None] | None"类型的参数分配给函数"__init__"中类型为"Cache | None"的参数"past_key_values"
```
...
return CausalLMOutputWithPast(
    logits = logits,
    past_key_values = past_key_values,  # ❌ 类型不匹配
...
return CausalLMOutputWithPast(
    logits = logits,
    past_key_values = cache,  # ✅ 使用 DynamicCache 类型
    hidden_states = hidden_states
)
```

**技术说明**:
1. **问题根源**: Transformers 库在新版本中引入了 `Cache` 抽象类，统一 KV 缓存接口
2. **解决方案**: 使用 `DynamicCache` 包装列表格式的 KV 缓存
3. **DynamicCache 特性**:
   - 动态管理多层 KV 缓存
   - 支持增量更新（适用于生成场景）
   - 与 Transformers 生成工具链完全兼容

**改进点**:
- ✅ 修复 Pylance 类型检查错误
- ✅ 保持内部实现的灵活性（仍使用列表格式）
- ✅ 符合 Transformers 库的标准接口
- ✅ 支持 `use_cache` 条件控制（仅在需要时转换）

**影响**:
- ✅ 与 Hugging Face 生成工具（`generate()`, `beam_search()` 等）完全兼容
- ✅ 类型检查通过，IDE 不再报错
- ✅ 为未来升级 Transformers 版本做好准备
